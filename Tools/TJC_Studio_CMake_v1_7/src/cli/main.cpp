...
struct Page { std::string hmi; std::string cpp; std::vector<Component> components; };
...
static std::string readAfter(const std::string& data, size_t pos)
{
    ...
    size_t start = pos;
    while (pos < data.size() && isIdChar((unsigned char)data[pos])) ++pos;
    if (pos <= start) return {};
...
static std::vector<Page> parse(const std::string& path)
{
    ...
    std::map<std::string,uint8_t> comps;
    ...
    for (auto& kv: comps)
    {
        ...
        if (legacy && convTypes.count(t)) continue;
...
int main(int argc, char** argv)
{
    ...
    h << "// Generated by TJC_Studio v1.7.0\n#ifndef " << cls << "_H\n#define " << cls << "_H\n\n#include <TJC_Display.h>\n\n";
    h << "namespace " << cls << "_ID\n{\n";
    for (auto& p: pages) for (auto& c: p.components) h << "    constexpr uint8_t " << upperFirst(c.cpp) << " = " << (int)c.id << ";\n";
    h << "}\n\n";
    ...
    for (auto& p: pages)
    {
        ...
        h << "    void show() { _lcd.page(" << (p.hmi == "page0" ? 0 : 1) << "); }\n";
        h << "    void handleEvent(const TJC_Event& event);\n\n";
        for (auto& c: p.components) h << "    " << cppClass(c.type) << " " << c.cpp << ";\n";
        h << "\nprivate:\n    TJC_Display& _lcd;\n};\n\n";
    }

    h << "class " << cls << "Controller\n{\npublic:\n";
    h << "    virtual void onEvent(const TJC_Event& event) {}\n";
    for (auto& p: pages) for (auto& c: p.components) h << "    virtual void on" << upperFirst(c.cpp) << "Release() {}\n";
    h << "};\n\n";

    h << "class " << cls << "\n{\npublic:\n";
    h << "    explicit " << cls << "(TJC_Display& lcd);\n";
    h << "    void processEvents();\n";
    h << "    void setController(" << cls << "Controller* controller) { _controller = controller; }\n";
    h << "    void showPage(uint8_t page) { _lcd.page(page); }\n\n";
    for (auto& p: pages) h << "    " << cls << "_" << p.cpp << " " << p.cpp << ";\n";
    h << "\nprivate:\n    TJC_Display& _lcd;\n    " << cls << "Controller* _controller = nullptr;\n};\n\n#endif\n";

    cpp << "// Generated by TJC_Studio v1.7.0\n#include \"" << cls << ".h\"\n\n";
    for (auto& p: pages)
    {
        std::string pc = cls + "_" + p.cpp;
        cpp << pc << "::" << pc << "(TJC_Display& lcd)\n    : _lcd(lcd)";
        for (auto& c: p.components)
            cpp << ",\n      " << c.cpp << "(lcd." << factory(c.type) << "(\"" << c.hmi << "\", " << cls << "_ID::" << upperFirst(c.cpp) << "))";
        cpp << "\n{\n}\n\n";
        cpp << "void " << pc << "::handleEvent(const TJC_Event& event)\n{\n";
        for (auto& c: p.components) cpp << "    " << c.cpp << ".handleEvent(event);\n";
        cpp << "}\n\n";
    }

    cpp << cls << "::" << cls << "(TJC_Display& lcd)\n    : _lcd(lcd)";
    for (auto& p: pages) cpp << ",\n      " << p.cpp << "(lcd)";
    cpp << "\n{\n}\n\n";
    cpp << "void " << cls << "::processEvents()\n{\n    _lcd.update();\n    TJC_Event event;\n    while (_lcd.read(event))\n    {\n";
    for (auto& p: pages) cpp << "        " << p.cpp << ".handleEvent(event);\n";
    cpp << "        if (_controller)\n        {\n            _controller->onEvent(event);\n";
    for (auto& p: pages) for (auto& c: p.components)
        cpp << "            if (" << p.cpp << "." << c.cpp << ".isRelease(event)) _controller->on" << upperFirst(c.cpp) << "Release();\n";
    cpp << "        }\n    }\n}\n";

    json << "{\\n  \\\"generator\\\": \\\"TJC_Studio v1.7.0\\\",\\n  \\\"components\\\": [\\n";
    bool first = true;
    for (auto& p: pages) for (auto& c: p.components)
    {
        if (!first) json << ",\\n";
